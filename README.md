...
Open your computer terminal in the folder that's storing the `.pem` file (This is the key pair we've downloaded from the previous step).
...
-  `chmod 400 minecraft_ssh.pem`: this will ensure your key is not publicly viewable.
...
We will use Amazon Corretto to get Java 17 installed, which will then run Minecraft. You can follow [Corretto 17 User Guide](https://docs.aws.amazon.com/corretto/latest/corretto-17-ug/amazon-linux-install.html) or just simply type the command below

`sudo yum install java-17-amazon-corretto`

After executing the command, it will download and install Java 17 on your server. Verify by typing `java --version`
...
Download the minecraft server file. See [here](https://www.minecraft.net/en-us/download/server)
...
After running the command, you probaly run into this error
...
Hit `i` to  enter insert mode, move to the line if `eula` and change to `eula=true`
...
If you see "DONE" which means your minecraft server has been started succesfully and now you are able to connect to your minecraft server

## Final step: Get your friends and join the server

Open your minecraft and type the server name and the public ipv4
